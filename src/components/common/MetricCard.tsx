...
export const MetricCard: React.FC<MetricCardProps> = ({
  ...
  const accentColors = {
    ...
    amber: "text-amber-400 bg-amber-100 border-amber-500/20",
  ...
  return (
    <div
      onClick={onClick}
      className={`p-4 rounded-xl bg-white border border-gray-200 transition-all duration-200 ${
        onClick ? "cursor-pointer hover:border-gray-300 hover:bg-gray-50 active:scale-[0.99]" : ""
      } ${className}`}
    >
      ...
      <div className="flex items-baseline gap-2">
        <span className="text-xl sm:text-2xl font-bold font-mono tracking-tight text-gray-900">{value}</span>
        {target !== undefined && (
